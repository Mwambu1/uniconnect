...
import { getGroupById, addUserToGroup, checkIfUserIsMember, getGroupPosts } from "@/lib/firebase/firestore/firestore"; // Import Firestore functions
  ...
  useEffect(() => {
    const fetchGroup = async () => {
      if (id) {
        ...
        if (sliceUser) {
          const isUserMember = await checkIfUserIsMember(id as string, sliceUser.userId);
        ...
        <div className="flex flex-col gap-3 xl:w-[45%]">
          <div className="flex flex-col gap-3 bg-white rounded-lg w-full">
            ...
            <div className="flex justify-end text-white p-3">
              {loading ? (
                <button className="rounded-lg bg-gray-400 shadow-xl py-2 w-32" disabled>
                  <h1 className="text-sm">Checking...</h1>
                </button>
              ) : isMember ? (
                <button className="rounded-lg bg-green-400 shadow-xl py-2 w-32" disabled>
            ...
            {loadingPosts ? (
              ...
              posts.map((post) => (
                <div key={post.post_id} className="mb-4">
                  <GroupPostComponent data={post} /> {/* Display each group post */}
                </div>
              ))
            )}
          </div>
