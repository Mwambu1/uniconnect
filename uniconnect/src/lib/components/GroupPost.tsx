...
import { BiCheckCircle, BiComment, BiDotsVerticalRounded } from "react-icons/bi";
import { HiHeart } from "react-icons/hi";
import { IoMdThumbsUp } from "react-icons/io";
import { MdCelebration } from "react-icons/md";
import { RiEmotionLaughFill } from "react-icons/ri";
import { GroupPost, User } from "../model/types";
import { arrayRemove, arrayUnion, doc, updateDoc } from "firebase/firestore";
import { db } from "../firebase/config";
import { useSelector } from "react-redux";
import { selectUser } from "../redux/slices/user/selectors";

export default function GroupPostComponent({ data }: { data: GroupPost }) {
  const [showReactions, setShowReactions] = useState(false);
  const user: User = useSelector(selectUser); // Get current user from Redux store
  const userId = user.userId;

  const [liked, setLiked] = useState(data.liked_by.includes(userId)); // Check if the user has already liked the post

  // Firestore path for group posts
  const postDocRef = doc(db, `group-posts`, data.post_id);
      ...
      <div className="flex justify-between p-2">
        <div className="flex justify-center items-center gap-2">
          <div className="relative w-10 h-10 rounded-full bg-gray-400 overflow-hidden">
            <Image
              src={data.media_url[0] || "/default_image.png"} // Display default image if no media
              alt="Post media"
              layout="fill"
              objectFit="cover"
            />
